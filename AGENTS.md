...
- `apps/server/` — executable TypeScript core; its one-listener Connect runtime implements Administrator setup and authentication, secure Pairing/Device persistence and protection, bundled-provider discovery and reconciliation, provider-type listing, candidate and exact-revision stored-instance connection tests, and verified provider-instance create/list/get/update/delete including disable and re-enable. Public DeviceService handlers, CLI and Apple pairing, and consumer behavior beyond manual endpoint verification remain unimplemented.
...
- Keep PostgreSQL-time decisions inside SQL or project them as booleans; a typed raw `sql<Date>` projection does not install the column timestamp decoder and can make Date/string comparisons fail open.
...
- Commit each accepted Pairing poll's PostgreSQL-time `next_poll_at` before surfacing an approved credential-delivery failure; never let the expected failure roll back the durable gate.
