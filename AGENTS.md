...
- Keep the OMP runner's own idle/deadline enforcement and PID handoff; Sandcastle `0.12.0` `noSandbox()` timeout races do not terminate the spawned shell and OMP process.
- Disable OMP Bash and Eval auto-backgrounding in the issue executor; daemon-supervised jobs can outlive a timed-out session and collide with the trusted host check.
- Treat failed issue-executor remote probes as unknown rather than branch absence; before removing a stale lock, make dead `running` metadata retryable, and resume an exact recorded draft's remaining publication transition without rerunning OMP.
- Keep the local `@standard-schema/spec` declaration while Sandcastle remains pinned to `0.12.0`; that release publishes a public type import without the type-only package, and Nama does not weaken `skipLibCheck`.
