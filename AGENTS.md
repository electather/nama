...
- Require the fast Actions, Go, JavaScript/TypeScript, and Python CodeQL jobs, but keep Swift CodeQL non-blocking and build it manually for one host-architecture macOS scheme destination; default Swift autobuild selects an unqualified Release target, rebuilds universal AetherEngine dependencies, and eventually fails dependency module resolution.
