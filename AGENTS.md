...
- Give the compiled provider-discovery process integration flow at least 60 seconds in CI; its full create, update, connection-test, disable, re-enable, and delete lifecycle can exceed Vitest's 30-second default on a cold Linux runner.
...
- Keep PR CI on a selected Actions allowlist that admits GitHub-owned actions and `jdx/mise-action`, require CodeQL results through the main ruleset's code-scanning gate, and run `mise run check:swift` on `macos-26`; the Swift check invokes `xcodebuild` and cannot run on Ubuntu.
