...
- `apps/ios/` — universal SwiftUI application and Swift Testing target; manual Nama endpoint normalization, explicit foreground `_nama._tcp` discovery, cancellable public setup-status verification, safe connection states, verified endpoint preference persistence and restoration, native Better Auth device authorization and refresh, endpoint-bound Keychain storage, native form/tvOS presentation, and the macOS outgoing-network sandbox are implemented. Media behavior remains unimplemented.
