...
- `plugins/jellyfin/` — first-party TypeScript provider adapter; its production executable implements private health, provider information, connection inspection, targeted normalized library and watch-state reads, and resumable best-effort catalog scans with bounded safe failures. It advertises `LIBRARY_READ`; watch-state scans and the remaining media capabilities are unimplemented.
