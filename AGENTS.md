...
- `plugins/jellyfin/` — first-party TypeScript provider adapter; its production executable implements private health, provider information, connection inspection, exact-instance targeted movie/episode watch-state reads, and bounded explicit watched/unwatched writes with ambiguity readback. It advertises `WATCHED_WRITE`; read capabilities remain unadvertised until their complete scans exist.
