...
- `apps/server/` — executable TypeScript core; its one-listener Connect runtime implements administrator setup and authentication over the durable Better Auth schema. Device pairing and consumer-client behavior remain unimplemented.
- `apps/cli/` — Go public-API client surface implementing profiles, administrator setup, sign-in, authentication status, human and JSON discovery, shell completion, and a compatibility-checked process contract.
