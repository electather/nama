...
- `plugins/jellyfin/` — first-party TypeScript provider adapter; its production executable implements private health, provider information, connection inspection, targeted normalized library reads, exact-instance targeted movie/episode watch-state reads, resumable best-effort catalog scans with bounded safe failures, anonymous public artwork resolution, and bounded explicit watched/unwatched writes with ambiguity readback. It advertises `LIBRARY_READ`, `ARTWORK_RESOLVE`, and `WATCHED_WRITE`; complete watch-state scans and the remaining media capabilities are unimplemented.
