...
- Keep issue #145 browser-free and CLI-uniform: `AuthService.ApproveDeviceAuthorization` must pass the authenticated Administrator session context to Better Auth's internal `deviceVerify` then `deviceApprove` APIs without public approval routes, loopback HTTP, or direct Better Auth persistence access; browser sign-in and confirmation belong to issue #167 and reuse the same internal application service.
...
- Issue #145 is the sole accepted pre-release exception to additive public Protobuf evolution: remove the unimplemented `DeviceService`, add only `AuthService.ApproveDeviceAuthorization` for Administrator approval, reserve removed fields and enum values where supported, regenerate every consumer in the same change, and advance the breaking baseline.
