...
it("encodes canonical authenticated provider page tokens as unpadded base64url", async () => {
  ...
  try {
    ...
    expect(payload.toString("utf8")).toBe(
      '{"cursor":"jellyfin","expires_at":900000,"method":"nama.api.v1.ProviderService.ListProviderTypes","page_size":1,"principal_id":"administrator-a","query":"{}","version":1}',
    );
    expect(
      codec.decode({
...
it("rejects tampering, expiry, key changes, and cross-query token reuse", async () => {
  ...
  try {
    ...
    expect(() => decode({ principalId: "administrator-b" })).toThrow();
