...
const CATALOG_SCAN_RESPONSE_MESSAGES = Object.freeze({
  forbidden: "Jellyfin catalog is forbidden",
  invalid: "Jellyfin catalog response is invalid",
  unavailable: "Jellyfin catalog is unavailable",
});
...
const normalizeCatalogObservation = (providerItem: unknown) => {
  if (!isUnknownRecord(providerItem) || typeof providerItem["Type"] !== "string") {
    throw jellyfinScanError("Jellyfin catalog response is invalid", Code.Internal, "INTERNAL");
  }
...
const catalogPageFromResponse = (response: JellyfinJsonResponse, pageSize: number) => {
  const providerItems = jellyfinScanBodyFromResponse(response, CATALOG_SCAN_RESPONSE_MESSAGES)[
    "Items"
  ];
