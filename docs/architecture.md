...
The implemented baseline is narrower than that topology. `@nama/server` currently runs one Effect application with one native listener, immutable configuration, reviewed Drizzle migrations, fail-closed initialization reconciliation over one PostgreSQL pool, readiness probing, exact health routes, and Setup and administrator Auth RPCs through a private Better Auth adapter. Better Auth routes remain unmounted; setup closes its durable marker transactionally; and sign-out confirms durable session revocation. The Go CLI implements profiles, administrator setup, sign-in, authentication status, complete human and JSON discovery, shell completion, documented exits, and semantic process-contract enforcement. Device pairing, Apple-client behavior, provider runtime, and playback behavior remain target contracts rather than implemented capabilities.
