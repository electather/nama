...
The complete provider-management tracer provisions a pinned disposable
Jellyfin server, creates explicit enabled and disabled users, and reaches it
through both candidate and exact persisted-instance launches of this production
plugin. Controlled HTTP fixtures retain deterministic coverage of redirect
refusal, cancellation, malformed and oversized responses, hostile remote
failures, and raw-response redaction.
