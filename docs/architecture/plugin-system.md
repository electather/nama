...
The supervisor keys current instance admission by opaque instance ID and exact revision. A replacement fences stale admission, drains bounded in-flight calls, and requires certain process-group cleanup before the next revision can launch. The future provider-management owner will load each transactional configuration and credential snapshot and hold its per-instance writer gate across this supervisor cutover so a stale database read cannot reopen an old revision. Playback sessions will outlive process incarnations through their opaque session context; synchronization runs will never mix instance revisions.
