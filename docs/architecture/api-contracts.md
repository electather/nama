...
5. Management RPCs may expose a provider type as runtime data, such as the neutral identifier `jellyfin`, because selecting and configuring an installed provider is the purpose of that API. This approved design narrows the unqualified provider-boundary wording in `architecture.md`: remote item IDs, SDK types, wire errors, and provider-specific consumer shapes still stop at the plugin, while an installed provider type is a Nama-managed resource. Provider types never appear in service or message names.
