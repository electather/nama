...
- process-isolated integration against disposable PostgreSQL with production migrations, prior-journal upgrade, constraints, the complete initialization state matrix, conditional repair failure, pool closure, and readiness loss/recovery; every active fork scope owns a distinct database, general files use two workers, and the shared real-Jellyfin cohort uses one non-overlapping serial worker with its restart-mutating proof last;
...
Integration PostgreSQL must use one isolated Compose project with a dynamically published host port and disposable volume. Every concurrent test scope owns a distinct database and terminates and drops only that exact database; tests must never touch the developer database. A compile-only check or generated Protobuf round trip is not server runtime proof.
