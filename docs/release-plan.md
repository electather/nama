...
- Better Auth remains private behind Nama-owned setup and CLI Administrator-authentication RPCs, while its OAuth authorization-server metadata, JWKS, device-approval, token, and revocation routes are public. Issue #145 exposes no browser email/password or session routes; issue #167 separately owns an optional browser approval surface. Neither the Go CLI nor the universal Apple app imports Better Auth implementation types.
