...
[ADR-0033](0033-better-auth-oauth-device-authorization.md) supersedes the prohibition on public Better Auth routes only for the fixed Apple client's standard OAuth authorization-server metadata, JWKS, device-code, token, refresh, and revocation endpoints. Nama setup, Administrator authentication, and CLI approval remain behind generated Connect contracts and the private adapter; browser email/password and session routes remain private unless issue #167 implements its separate web approval surface.
